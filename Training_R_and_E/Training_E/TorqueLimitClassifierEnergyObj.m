function [Torque_Manageable,act,eT,MusclesOI,Energy_Value] = TorqueLimitClassifierEnergyObj(Model_OS, DOF, q, qdot, tau)
...
for m = 1:M
    [~, ~, FMltilde(:,m), FMvtilde(:,m), Fpe(:,m), cos_alpha(:,m), energy_term(:,m)] = HillModel_RigidTendon_WithEnergyObj(act(:,m),LMT(:,m),VMT(:,m),MusclesOI(m),ActiveFVParameters,PassiveFLParameters,Faparam);
end
...
Energy_Value = sum(act .* energy_term, 2);
end
...
%f     = 0.5 * sum(x.^2);
[M, N, nDof, Fmax, Fpas, ID_data, MomentArm, energy_term] = deal(auxdata{:});
x_opt = reshape(x, M+nDof, N)';
f     = sum(sum( energy_term .* x_opt(:,1:M) )) + 0.5*1000*sum(sum( x_opt(:,M+1:end).^2 ));  % We want to minimize the optimization vector (activations of muscles and reserves)
% f = f/100.0;
...
[M, N, nDof, Fmax, Fpas, ID_data, MomentArm,energy_term] = deal(auxdata{:});
x_opt = reshape(x, M+nDof, N)';
gtemp = [energy_term 1000*x_opt(:,M+1:end)];   % Gradient of the objective to the variables
% gtemp(gtemp==0)=1;
I = N*(M+nDof);
g = reshape(gtemp',I,1);
% g = g/100.0;
...
% [M, N, nDof, Fmax, Fpas, ID_data, MomentArm, energy_term] = deal(auxdata{:});
% x_opt = reshape(x, M+nDof, N)';
% f     = 0.5 * (sum(sum(energy_term .* x_opt(:,1:M).^2)) + 10*sum(sum(x_opt(:,M+1:end).^2)));  % We want to minimize the optimization vector (activations of muscles and reserves)
% % NOTE: We especially want to minimize the reserves to see whether the
% % muscles can deliver the desired moment since the activations of the
% % reserves are the torques itself and the activations of the muscles scale
% % with an optimal force we don't need to put an extremely high weight on the term
...
% [M, N, nDof, Fmax, Fpas, ID_data, MomentArm, energy_term] = deal(auxdata{:});
% x_opt = reshape(x, M+nDof, N)';
% gtemp = [energy_term .* x_opt(:,1:M) 10*x_opt(:,M+1:end)];   % Gradient of the objective to the variables
% I = N*(M+nDof);
% g = reshape(gtemp',I,1);
% end
% % ------------------------------------------------------------------
