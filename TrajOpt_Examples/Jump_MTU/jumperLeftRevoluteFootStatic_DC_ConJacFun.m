function jac = jumperLeftRevoluteFootStatic_DC_ConJacFun(X, auxdata)
...
sv_mat = zeros(N, Ncontrols*(Ncontrols*2));
parfor j = 1:N
    parState_mus_i = parModels_mus.Value.updWorkingState();
    si_t = zeros(1,Ncontrols*(Ncontrols*2));
    sj_t = zeros(1,Ncontrols*(Ncontrols*2));
    sv_t = zeros(1,Ncontrols*(Ncontrols*2));
    sp_t = 1;
    for k = 1:Ncontrols*2
        pstate = states(j,ctrl_states)';
        pstate(k) = pstate(k) + del; % forward diff
        [ptau,~] = ComputeMuscleTorques(parModels_mus.Value, parState_mus_i, MusclesOI, CoordinatesOI, DOF, act(j,:)', pstate(1:Ncontrols), pstate(Ncontrols+1:end));
        pceq_mus = (ptau - controls(j,:)' - ceq_mus_mat(:,j)) / del; 
...
sj = [sj sj_mat(:)'];
sv = [sv sv_mat(:)'];
gradceq_mus = sparse(sj, si, sv, Ncontrols*N, length(X)); % ipopt transpose from fmincon
