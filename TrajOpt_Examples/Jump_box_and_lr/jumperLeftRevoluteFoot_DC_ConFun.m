function c = jumperLeftRevoluteFoot_DC_ConFun(X,auxdata)
%
...
if (addNN)
    ...
    cn_mynn = zeros(Ncontrols*N,1);
    %[N123(t=1), (t=2), (t=3),...]
    for i = 1:N
        % ankle, knee, hip
%         y = NN_FD_tanh([states(i,[2:4 6:8]) controls(i,:)]',Wall,Ball);
        y = NN_FD([states(i,[2:4 6:8]) controls(i,:)]',Wall,Ball,tor_limit);
        cn_mynn(Ncontrols*(i-1)+1 : Ncontrols*i) = y;
    end
    cn = [cn;cn_mynn];
end
