function jac = jumperLeftRevoluteFoot_DC_ConJacFun(X, auxdata)
...
if (addNN)
    ...
    Nsc = Ncontrols*3;
    len = Ncontrols*N*Nsc; % *3 means u,dq,q
    SI = zeros(1,len);
    SJ = zeros(1,len);
    SV = zeros(1,len);
    SP = 1;
...
%         dydx = NN_BP_tanh([states(i,[2:4 6:8]) controls(i,:)]',Wall,Ball,tor_limit);
        dydx = NN_BP([states(i,[2:4 6:8]) controls(i,:)]',Wall,Ball,tor_limit);
        for j = 1 : Ncontrols
            SI(SP:SP+Nsc-1) = ([2:4 6:8 9:11]-1)*N + i;
            SJ(SP:SP+Nsc-1) = Ncontrols*(i-1) + j;
            SV(SP:SP+Nsc-1) = dydx(j,:)';
            SP = SP+Nsc;
        end
    end
    gradcn_mynn = sparse(SJ,SI,SV, Ncontrols*N, (Nstates+Ncontrols)*N);
