...
if (Ncontrols ~= 4) 
    disp('Ncontrols should be 4 for lower limb');
    return
end

if (Ncoord ~= 5)
    disp('Ncoord should be 5 for lower limb revolute');
    return
end
...
musModel.initSystem();
DOF = {'hip_flexion_l' 'knee_angle_l' 'ankle_angle_l'};
...
nDOF = length(DOF);
options.cl = [options.cl; zeros(nDOF*N,1)];
options.cu = [options.cu; zeros(nDOF*N,1)];
...
options.ipopt.acceptable_constr_viol_tol = 3.0;
options.ipopt.acceptable_obj_change_tol = 1e-3;
options.ipopt.acceptable_tol = 20.0;
options.ipopt.acceptable_iter = 10;

% The callback functions.
funcs.objective         = @swingStaticMuscle_DC_ObjFun;
funcs.constraints       = @swingStaticMuscle_DC_ConFun;
funcs.gradient          = @swingStaticMuscle_DC_ObjGradFun;
funcs.jacobian          = @swingStaticMuscle_DC_ConJacFun;
funcs.jacobianstructure = @swingStaticMuscle_DC_ConJacStructFun;
...
save(['activations',char(date),'_3.0s_N200_se80.mat'],'act')
...
StatesData = struct();
StatesData.name = [char(osimModel.getName()), '_Optimal_States_', char(date),'_3.0s_N200_se80_staticMus'];
...
ControlData = struct();
ControlData.name = [char(osimModel.getName()), '_Optimal_Controls_', char(date),'_3.0s_N200_se80_staticMus'];
