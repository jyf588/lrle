...
addNN = true;   % should always set to true (always use lr)
tor_limit_tol = 5.0; % Nm
useMeta = true;
...
Wall_E = {};
Ball_E = {};
filename = '2392/leg_torque_2D_E_tilde_q-1224&-2501&-1211_dq5&15&15_tau200250250_2392_apr8_512360180elu_mse_ediv10_r4e.mat';
for i = 0:3
    Wname = strcat('W',num2str(i));
    W = load(filename, Wname);
    Wall_E{end+1} = W.(Wname);
    Bname = strcat('B',num2str(i));
    B = load(filename, Bname);
    Ball_E{end+1} = B.(Bname);
end
auxdata.useMeta = useMeta;
auxdata.Wall_E = Wall_E;
auxdata.Ball_E = Ball_E;
...
options.cl = [zeros((N-1)*Nstates + Nstates,1); 0]; % first (N-1)*Nstates + Nstates ceq + final vel
options.cu = [zeros((N-1)*Nstates + Nstates,1); 0];
if (addNN)
...
options.ipopt.tol              = 1e-6;  % never reached, solve max (500) iter for both
...
StatesData = struct();
StatesData.name = [char(osimModel.getName()), '_Optimal_States_', char(date), '_', num2str(duration), 's_N', num2str(N), '_se80'];
if (useMeta)
    StatesData.name = [StatesData.name, '_meta'];
else
    StatesData.name = [StatesData.name, '_lronly'];
end
...
ControlData = struct();
ControlData.name = [char(osimModel.getName()), '_Optimal_Controls_', char(date), '_', num2str(duration), 's_N', num2str(N), '_se80'];
if (useMeta)
    ControlData.name = [ControlData.name, '_meta'];
else
    ControlData.name = [ControlData.name, '_lronly'];
end
